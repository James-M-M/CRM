...
public class BlobStorageService : IBlobStorageService
{
    ...
    private string blobContainerName = "crm";

    public BlobStorageService(IConfiguration configuration, ILogger<BlobStorageService> logger)
    {
        _configuration = configuration;
        _logger = logger;
        blobStorageconnection = _configuration.GetConnectionString("StorageConnectionString");
    }
    public async Task<string> UploadFileToBlobAsync(string strFileName, string contecntType, Stream fileStream)
    {
        try
        {
            ...
            _logger.LogInformation($"Uploading blob with name: {strFileName}");
